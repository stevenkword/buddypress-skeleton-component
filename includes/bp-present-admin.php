...
/**
 * bp_present_add_admin_menu()
 *
 * This function will add a WordPress wp-admin admin menu for your component under the
 * "BuddyPress" menu.
 */
function bp_present_add_admin_menu() {
	global $bp;

	if ( !is_super_admin() )
		return false;

	add_submenu_page( 'bp-general-settings', __( 'Example Admin', 'bp-present' ), __( 'Example Admin', 'bp-present' ), 'manage_options', 'bp-present-settings', 'bp_present_admin' );
}
...
add_action( bp_core_admin_hook(), 'bp_present_add_admin_menu' );
...
?>
	<div class="wrap">
		<h2><?php _e( 'Example Admin', 'bp-present' ) ?></h2>
		<br />

		<?php if ( isset($updated) ) : ?><?php echo "<div id='message' class='updated fade'><p>" . __( 'Settings Updated.', 'bp-present' ) . "</p></div>" ?><?php endif; ?>

		<form action="<?php echo site_url() . '/wp-admin/admin.php?page=bp-present-settings' ?>" name="example-settings-form" id="example-settings-form" method="post">

			<table class="form-table">
				<tr valign="top">
					<th scope="row"><label for="target_uri"><?php _e( 'Option One', 'bp-present' ) ?></label></th>
					<td>
						<input name="example-setting-one" type="text" id="example-setting-one" value="<?php echo esc_attr( $setting_one ); ?>" size="60" />
					</td>
				</tr>
					<th scope="row"><label for="target_uri"><?php _e( 'Option Two', 'bp-present' ) ?></label></th>
					<td>
			...
			<p class="submit">
				<input type="submit" name="submit" value="<?php _e( 'Save Settings', 'bp-present' ) ?>"/>
			</p>
 ...
 * @since 1.6
 */
function bp_present_install_tables() {
	global $wpdb;
	 ...
	 */
	$sql = array();
	$sql[] = "CREATE TABLE IF NOT EXISTS {$wpdb->base_prefix}bp_present (
	 ...
	 * the table. (Make sure you increment the BP_PRESENT_DB_VERSION constant though).
	 */
	dbDelta($sql);

	update_site_option( 'bp-present-db-version', BP_PRESENT_DB_VERSION );
}
//add_action( 'admin_init', 'bp_present_install_tables' );
?>