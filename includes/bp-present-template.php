 ...
 * convention is usually something like 'bp_present_get_item_name()' for the function that returns
 * the value, and 'bp_present_item_name()' for the function that echoes.
 */
 ...
 * purpose. See bp-present-classes.php for more details.
 *
 */

function bp_present_has_items( $args = '' ) {
...
function bp_present_the_item() {
	global $items_template;
	return $items_template->query->the_post();
}

function bp_present_item_name() {
	echo bp_present_get_item_name();
}
	/* Always provide a "get" function for each template tag, that will return, not echo. */
	function bp_present_get_item_name() {
		global $items_template;
		echo apply_filters( 'bp_present_get_item_name', $items_template->item->name ); // Example: $items_template->item->name;
	}
 ...
 * @since 1.6
 */
function bp_present_pagination_count() {
	echo bp_present_get_pagination_count();
}
	/**
	 ...
	 * @since 1.6
	 */
	function bp_present_get_pagination_count() {
		global $items_template;

		$pagination_count = sprintf( __( 'Viewing page %1$s of %2$s', 'bp-present' ), $items_template->query->query_vars['paged'], $items_template->query->max_num_pages );

		return apply_filters( 'bp_present_get_pagination_count', $pagination_count );
	}
 ...
 * @since 1.6
 */
function bp_present_item_pagination() {
	echo bp_present_get_item_pagination();
}
	/**
	 ...
	 * @since 1.6
	 */
	function bp_present_get_item_pagination() {
		global $items_template;
		return apply_filters( 'bp_present_get_item_pagination', $items_template->pag_links );
	}
 ...
 * @since 1.6
 */
function bp_present_high_fiver_avatar( $args = array() ) {
	echo bp_present_get_high_fiver_avatar( $args );
}
	/**
	 ...
	 */
	function bp_present_get_high_fiver_avatar( $args = array() ) {
		$defaults = array(
 ...
 * @since 1.6
 */
function bp_present_high_five_title() {
	echo bp_present_get_high_five_title();
}
	/**
	 ...
	 * @since 1.6
	 */
	function bp_present_get_high_five_title() {
		...
		$recipient_id    = get_post_meta( get_the_ID(), 'bp_present_recipient_id', true );
		$recipient_link  = bp_core_get_userlink( $recipient_id );

		// Use sprintf() to make a translatable message
		$title 		 = sprintf( __( '%1$s gave %2$s a high-five!', 'bp-present' ), $high_fiver_link, $recipient_link );

		return apply_filters( 'bp_present_get_high_five_title', $title, $high_fiver_link, $recipient_link );
	}
 ...
 * @since 1.6
 */
function bp_present_total_high_five_count() {
	echo bp_present_get_total_high_five_count();
}
	/**
	 ...
	 * @return int
	 */
	function bp_present_get_total_high_five_count() {
		$high_fives = new BP_Example_Highfive();
		$high_fives->get();

		return apply_filters( 'bp_present_get_total_high_five_count', $high_fives->query->found_posts, $high_fives );
	}
 ...
 * @since 1.6
 */
function bp_present_total_high_five_count_for_user( $user_id = false ) {
	echo bp_present_get_total_high_five_count_for_user( $user_id = false );
}
	/**
	 ...
	 * @return int
	 */
	function bp_present_get_total_high_five_count_for_user( $user_id = false ) {
