 ...
 * @since 1.6
 */
function bp_present_directory_setup() {
...
function bp_present_screen_one() {
	...
	do_action( 'bp_present_screen_one' );
	 ...
	 *
	 * In an earlier function bp_present_load_template_filter() we set up a filter on the core BP template
	...
	bp_core_load_template( apply_filters( 'bp_present_template_screen_one', 'example/screen-one' ) );
...
//	add_action( 'bp_template_title', 'bp_present_screen_one_title' );
//	add_action( 'bp_template_content', 'bp_present_screen_one_content' );
...
//	bp_core_load_template( apply_filters( 'bp_core_template_plugin', 'members/single/plugins' ) );
	 ...
	 */
	function bp_present_screen_one_title() {
		_e( 'PresentationsSteven', 'bp-present' );
	}

	function bp_present_screen_one_content() {
		global $bp;

		$high_fives = bp_present_get_highfives_for_user( $bp->displayed_user->id );
		 ...
		 * knowledge or intent.
		 */
		$send_link = wp_nonce_url( $bp->displayed_user->domain . $bp->current_component . '/screen-one/send-h5', 'bp_present_send_high_five' );
	?>
		<h4><?php _e( 'Welcome to Screen One', 'bp-present' ) ?></h4>
		<p><?php printf( __( 'Send %s a <a href="%s" title="Send high-five!">high-five!</a>', 'bp-present' ), $bp->displayed_user->fullname, $send_link ) ?></p>

		<?php if ( $high_fives ) : ?>
			<h4><?php _e( 'Received High Fives!', 'bp-present' ) ?></h4>
 ...
 */
function bp_present_screen_settings_menu() {
	global $bp, $current_user, $bp_settings_updated, $pass_error;

	if ( isset( $_POST['submit'] ) ) {
		/* Check the nonce */
		check_admin_referer('bp-present-admin');
		 ...
		 */
		update_user_meta( $bp->loggedin_user->id, 'bp-present-option-one', attribute_escape( $_POST['bp-present-option-one'] ) );
	}

	add_action( 'bp_template_content_header', 'bp_present_screen_settings_menu_header' );
	add_action( 'bp_template_title', 'bp_present_screen_settings_menu_title' );
	add_action( 'bp_template_content', 'bp_present_screen_settings_menu_content' );

	bp_core_load_template( apply_filters( 'bp_core_template_plugin', 'members/single/plugins' ) );
}

	function bp_present_screen_settings_menu_header() {
		_e( 'Example Settings Header', 'bp-present' );
	}

	function bp_present_screen_settings_menu_title() {
		_e( 'Example Settings', 'bp-present' );
	}

	function bp_present_screen_settings_menu_content() {
		global $bp, $bp_settings_updated; ?>

		<?php if ( $bp_settings_updated ) { ?>
			<div id="message" class="updated fade">
				<p><?php _e( 'Changes Saved.', 'bp-present' ) ?></p>
			</div>
		<?php } ?>

		<form action="<?php echo $bp->loggedin_user->domain . 'settings/example-admin'; ?>" name="bp-present-admin-form" id="account-delete-form" class="bp-present-admin-form" method="post">

			<input type="checkbox" name="bp-present-option-one" id="bp-present-option-one" value="1"<?php if ( '1' == get_user_meta( $bp->loggedin_user->id, 'bp-present-option-one', true ) ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Do you love clicking checkboxes?', 'bp-present' ); ?>
			<p class="submit">
				<input type="submit" value="<?php _e( 'Save Settings', 'bp-present' ) ?> &raquo;" id="submit" name="submit" />
			</p>

			<?php
			/* This is very important, don't leave it out. */
			wp_nonce_field( 'bp-present-admin' );
			?>
