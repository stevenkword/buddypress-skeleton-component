...
/**
 * bp_present_screen_notification_settings()
 *
 ...
 */
function bp_present_screen_notification_settings() {
	global $current_user;
	...
	?>
	<table class="notification-settings" id="bp-present-notification-settings">

		<thead>
		<tr>
			<th class="icon"></th>
			<th class="title"><?php _e( 'Example', 'bp-present' ) ?></th>
			<th class="yes"><?php _e( 'Yes', 'bp-present' ) ?></th>
			<th class="no"><?php _e( 'No', 'bp-present' )?></th>
		</tr>
		</thead>
		...
		<tr>
			<td></td>
			<td><?php _e( 'Action One', 'bp-present' ) ?></td>
		...
		<tr>
			<td></td>
			<td><?php _e( 'Action Two', 'bp-present' ) ?></td>
		...
		<?php do_action( 'bp_present_notification_settings' ); ?>

		</tbody>
	</table>
<?php
}
add_action( 'bp_notification_settings', 'bp_present_screen_notification_settings' );


/**
 * bp_present_remove_screen_notifications()
 *
 * Remove a screen notification for a user.
 */
function bp_present_remove_screen_notifications() {
	global $bp;
...
add_action( 'bp_present_screen_one', 'bp_present_remove_screen_notifications' );
add_action( 'xprofile_screen_display_profile', 'bp_present_remove_screen_notifications' );


/**
 * bp_present_format_notifications()
 *
 ...
 * examples of usage.
 */
function bp_present_format_notifications( $action, $item_id, $secondary_item_id, $total_items ) {
	global $bp;
	...
	switch ( $action ) {
		case 'new_high_five':
			...
			if ( (int)$total_items > 1 ) {
				return apply_filters( 'bp_present_multiple_new_high_five_notification', '<a href="' . $bp->loggedin_user->domain . $bp->example->slug . '/screen-one/" title="' . __( 'Multiple high-fives', 'bp-present' ) . '">' . sprintf( __( '%d new high-fives, multi-five!', 'bp-present' ), (int)$total_items ) . '</a>', $total_items );
			} else {
				$user_fullname = bp_core_get_user_displayname( $item_id, false );
				$user_url = bp_core_get_user_domain( $item_id );
				return apply_filters( 'bp_present_single_new_high_five_notification', '<a href="' . $user_url . '?new" title="' . $user_fullname .'\'s profile">' . sprintf( __( '%s sent you a high-five!', 'bp-present' ), $user_fullname ) . '</a>', $user_fullname );
			}
		break;
	}

	do_action( 'bp_present_format_notifications', $action, $item_id, $secondary_item_id, $total_items );
...
function bp_present_send_high_five_notification( $to_user_id, $from_user_id ) {
	global $bp;
	...
	$subject = '[' . get_blog_option( 1, 'blogname' ) . '] ' . sprintf( __( '%s high-fived you!', 'bp-present' ), stripslashes($sender_name) );
...
---------------------
', 'bp-present' ), $sender_name, $sender_name, $sender_profile_link, $sender_name, $sender_highfive_link );

	$message .= sprintf( __( 'To disable these notifications please log in and go to: %s', 'bp-present' ), $receiver_settings_link );

	// Send it!
	wp_mail( $to, $subject, $message );
}
add_action( 'bp_present_send_high_five', 'bp_present_send_high_five_notification', 10, 2 );
