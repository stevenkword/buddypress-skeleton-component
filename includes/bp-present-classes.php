...
class BP_Example_Highfive {
	...
	function save() {
		 ...
		 * filter to 'bp-present-filters.php'
		 *
		 *   add_filter( 'example_data_fieldname1_before_save', 'wp_filter_kses' );
		 */

		$this->high_fiver_id = apply_filters( 'bp_present_data_high_fiver_id_before_save', $this->high_fiver_id, $this->id );
		$this->recipient_id  = apply_filters( 'bp_present_data_recipient_id_before_save', $this->recipient_id, $this->id );
		$this->date	     = apply_filters( 'bp_present_data_date_before_save', $this->date, $this->id );

		// Call a before save action here
		do_action( 'bp_present_data_before_save', $this );
		...
		if ( $this->id ) {
			...
			$wp_update_post_args = array(
				...
				'post_title'	=> sprintf( __( '%1$s high-fives %2$s', 'bp-present' ), bp_core_get_user_displayname( $this->high_fiver_id ), bp_core_get_user_displayname( $this->recipient_id ) )
			);
			...
			if ( $result ) {
				update_post_meta( $result, 'bp_present_recipient_id', $this->recipient_id );
			}
		} else {
			// Set up the arguments for wp_insert_post()
		...
		do_action( 'bp_present_data_after_save', $this );
	...
	/**
	 * Part of our bp_present_has_high_fives() loop
	 *
	 * @package BuddyPress_Skeleton_Component
	 * @since 1.6
	 */
