 ...
 * @since 1.6
 */
function bp_present_high_five_save() {
	...
	if ( bp_is_example_component() && bp_is_current_action( 'screen-one' ) && bp_is_action_variable( 'send-h5', 0 ) ) {
		...
		if ( bp_is_my_profile() ) {
			...
			bp_core_add_message( __( 'No self-fives! :)', 'bp-present' ), 'error' );
		} else {
			if ( bp_present_send_highfive( bp_displayed_user_id(), bp_loggedin_user_id() ) )
				bp_core_add_message( __( 'High-five sent!', 'bp-present' ) );
			else
				bp_core_add_message( __( 'High-five could not be sent.', 'bp-present' ), 'error' );
		}

		bp_core_redirect( bp_displayed_user_domain() . bp_get_example_slug() . '/screen-one' );
	}
}
add_action( 'bp_actions', 'bp_present_high_five_save' );
